...
import { deleteVideo, getSpecificVideo, getUserInfo, recommnededFeed } from "../utils/fetchData";
...
const VideoDetails = () => {
  ...
  const textColor = useColorModeValue("white.900", "white.50");
  ...
  useEffect(() => {
    ...
    getSpecificVideo(firestoreDb, videoId).then((data) => {
      setVideoInfo(data);
      // console.log(data);
      recommnededFeed(firestoreDb , data.category , videoId).then((recommended) => {
        setFeeds(recommended)
      })
